    ...
    <UserData
      ...
      render={(userData) => (
        <div className="min-h-screen flex flex-col items-start justify-start bg-gray-50 px-4 md:px-10 pt-0">
          <h2 className="w-full flex justify-left mt-4 text-2xl md:text-3xl">
          ...
          {userData.shopShippingAddress ? (
            <p className="w-full flex justify-left text-gray-700 text-sm">
              {userData.shopShippingAddress.city},{" "}
              {userData.shopShippingAddress.state}
            </p>
          ) : null}
          {userData.shopDescription ? (
            <p className="w-full md:w-1/2 flex justify-left mt-4 text-gray-700 text-sm">
              {userData.shopDescription}
            </p>
          ) : null}
          <button
          ...
          {composeData && (
            <NewMessage
              ...
              onMessageSent={() => setComposeData(null)}
            />
          )}
          <DisplayGrid
            products={products}
